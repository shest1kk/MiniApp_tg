...
const MeetupTiming = () => {
  ...
  const timingDataDay1 = [
    {
      ...
      speaker: ["Чижов Алексей", "Ткачева Светлана"],
    },
    {
      time: "15:00",
      topic:
    ...
    {
      time: "16:50",
      topic: "\nЗаложи базу.<br/>Потом — выжи-<br/>май максимум.<hr>Как ставить рекорды по выручке\n\n",
    ...
    { time: "17:40", topic: "Перерыв", speaker: "" },
    {
      time: "17:55",
      topic:
        "\nЛьвы и овцы.<br/>Охота - как стиль жизни и работы.<hr>Как можно выполнить план по ТО.\n\n",
  ...
  const timingDataDay2 = [
    ...
    {
      time: "12:05",
      topic: "Вопреки внешним обстоятель-\nствам",
      speaker: "Егоров Александр, Дёмин Владислав, Фионина Ольга",
    },
    {
      time: "12:25",
      topic: "Вопреки рынку труда",
      speaker: "Бурыкина Анастасия, Юдин Сергей, Подлегаева Дарья",
    },
    {
      time: "12:55",
  ...
  const speakersData = {
    "Чижов Алексей": {
      image: "/assets/1.png",
      text: "CEO, Алексей Чижов поделится секретом успеха ЭйКей и что позволяет Компании быть лучшими.<hr>А так же расскажет, почему Эйнштейн стал нашим выбором для SSM в этом году, и что общего между выдающимся физиком и нашей Компанией?",
    },
    "Ткачева Светлана": {
      image: "/assets/1.png",
    ...
    "Сушко Павел": {
      ...
      text: "Информация о Павле Сушко...",
    },
  };

  const openModal = (speakerName) => {
    const data = speakersData[speakerName];
    if (data) {
      setModalData(data);
    }
  };
  ...
  const formatSpeakers = (speakers) => {
    // Если speakers не является массивом, преобразуем его в массив строк
    if (!Array.isArray(speakers)) {
      speakers = speakers.split("<hr>");
    }
    ...
    return speakers.map((speaker, index) => {
      ...
      return (
        <React.Fragment key={index}>
          {index > 0 && <hr />} {/* Вставляем <hr> только между элементами, а не перед первым */}
          <div>
            <a
              // href="/faq"
              onClick={(e) => {
